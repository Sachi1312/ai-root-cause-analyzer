...
**RAG over Incident History** — Retrieved past incidents become context for the LLM. Instead of guessing, the AI reasons from real historical data. Grounded responses, not hallucinations.
...
**Severity Scoring (P1–P5)** — Programmatic scoring based on anomaly severity, error count, number of services affected, and exception types. Mirrors real-world SRE practice at Google, Netflix, and Stripe.

**Log Compression** — Logs can be 50,000+ characters. The compressor prioritises error lines, warnings, and stack traces to fit within the LLM context window.
...
- HTML/CSS/JS — warm editorial beige UI design
...
```
git clone https://github.com/Sachi1312/ai-root-cause-analyzer.git
...
Anomaly Detector → spikes, cascades, OOM, pool exhaustion, timeouts
   ↓
Severity Scorer → P1 Critical to P5 Info
   ↓
Log Compressor → prioritise critical lines for LLM context window
   ↓
FAISS RAG → top 3 similar past incidents by cosine similarity
   ↓
